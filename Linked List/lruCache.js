...
var LRUCache = function (limit) {
  var cache = {};
  cache = Object.create(LRUCache.prototype);
  cache.storage = new List()
  cache.size = limit;
  return cache;
};

var LRUCacheItem = function (val, key) {
  var item = new ListNode(this.prev, [key, val], this.next)
  return item
};
...
LRUCache.prototype.get = function (key) {
  while (this.storage.head.next.val[0])
  if (this.storage.next === null) {
    return "cannot find value"
  } 
};

LRUCache.prototype.set = function (key, val) {
  this.storage.head = LRUCacheItem(val, key);
  this.storage.tail = LRUCacheItem(val, key);
